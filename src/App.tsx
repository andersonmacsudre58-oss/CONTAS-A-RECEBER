        ...
        if (rows.length > 0) {
          const formatted = rows.slice(1)
            .map((row: any[], index: number) => ({
              ...
              tipoCusteio: row[10],
              houveParcela: row[11],
              quantidadeParcelas: parseInt(row[12]) || 1,
              dataRecebimento2: row[13],
              valorRecebido2: parseFloat(row[14]) || 0,
              dataRecebimento3: row[15],
              valorRecebido3: parseFloat(row[16]) || 0,
              dataRecebimento4: row[17],
